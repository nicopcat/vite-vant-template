<template>
  <div class="content">
    <van-form ref="formRef" @submit="onSubmit" input-align="right" validate-first scroll-to-error>
      ...
      <div class="mt-4 p-2">
        <van-button round block type="primary" native-type="submit"> 修 改 </van-button>
      </div>
    </van-form>
  </div>
</template>
...
import { getEamDeviceSpotInspectionDetail, editEamDeviceSpotInspection } from '@/api/eam/deviceSpotInspection'
...
async function getDetail() {
  if (history.state?.id) {
    try {
      const { data } = await getEamDeviceSpotInspectionDetail(history.state?.id)
      editData.value = data
    } catch (error) {}
  }
}
...
async function onSubmit() {
  ...
  formRef.value
    ...
    .then(async () => {
      try {
        const { code, msg } = await editEamDeviceSpotInspection(editData.value)
